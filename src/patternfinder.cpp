...
void usage() {
    ...
    cerr << "Descriptions: Reads an encoded corpus and extracts patterns above a certain threshold. The patterns consist out of n-grams and skip-grams up to a certain maximum value of n. The resulting model will be built in-memory (very memory intensive) and subsequently stored in a binary representation on disk. If a class-file is specified, the output will immediately afterwards be decoded for you as well. Or decode an existing model using the -d flag." << endl;
    cerr << "Options:" << endl;
