    ...
    def build_moses_classifiers(self):
        ...
        if self.COLIBRI_GLOBALKEYWORDS:
            ...
            if not ('-I' in self.MOSES_CLASSIFIER_OPTIONS):
                if (len(list(glob.glob(self.WORKDIR + '/classifier.*.ibase'))) == 0):
...
WordPenalty: -0.5\n""")
        ...
        if self.BUILD_MOSES_MERT and self.MOSES_MERT_RUNS > 1:
            for i in range(1,self.MOSES_MERT_RUNS+1):
                ...
                if r:
                    ...
                    self.log("Scoring produces errors (need not be fatal)")
                os.rename(self.WORKDIR +'/summary.score',self.WORKDIR + '/summary-mert'+str(i)+'.score')
    ...
    def score(self, sourcefile, reffile, targetfile, inmertrun=False):
        ...
        f.write(s+ "\n")
        print >>sys.stderr, s
        s = str(self.bleu) + " " + str(self.meteor) + " " + str(self.nist)  + " " + str(self.ter) + " " + str(self.wer)  + " " + str(self.per)
        f.write(s + "\n")
        print >>sys.stderr, s
        f.close()
