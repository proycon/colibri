...
class MTWrapper(object):
    defaults = [
            ...
            ('BUILD_SRILM_SOURCEMODEL',False,''),
            ('BUILD_SRILM_TARGETMODEL',False,''),
            ('BUILD_GIZA_WORDALIGNMENT',False,''),
            ('BUILD_GIZA_WORDALIGNMENT_REV',False,''),
            ('BUILD_GIZA_WORDALIGNMENT_COOC',False,''),
    ...
    def build_giza_wordalignment(self):
        ...
        if not self.runcmd(self.EXEC_GIZA_SNT2COOC + ' ' + self.gettargetfilename('vcb') + ' ' + self.getsourcefilename('vcb') + ' ' + self.getsourcefilename('txt') + ' > ' + self.gets2tfilename('cooc'), 'GIZA++ Co-occurrence output',  self.gets2tfilename('cooc')): return False
    ...
    def build_giza_wordalignment_rev(self):
        ...
        if not self.runcmd(self.EXEC_GIZA_SNT2COOC + ' ' + self.getsourcefilename('vcb') + ' ' + self.gettargetfilename('vcb') + ' ' + self.gettargetfilename('txt') + ' > ' + self.gett2sfilename('cooc'), 'GIZA++ Co-occurrence output (reversed)',  self.gett2sfilename('cooc')): return False        
    ...
    def build_moses_wordtranstable(self):
        if not self.runcmd(self.EXEC_MOSES_WORDTRANSTABLE + ' ' + self.getsourcefilename('txt') + ' ' + self.gettargetfilename('txt') + ' ' + self.gets2tfilename('bal')  + ' ' + self.gets2tfilename(),'Build of Lexical Translation Table', self.gets2tfilename('s2t'), self.gets2tfilename('t2s')): return False 
        return True
