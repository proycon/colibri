...
class MTWrapper(object):
    ...
    def build_colibri_alignment(self):
        ...
        if self.BUILD_COLIBRI_GIZA:
            if not self.runcmd(self.EXEC_COLIBRI_ALIGNER + ' -s ' + self.getsourcefilename('graphpatternmodel.colibri') + ' -t ' + self.gettargetfilename('graphpatternmodel.colibri') + ' -o ' + self.gets2tfilename('alignmodel.colibri') + ' -N ' + self.COLIBRI_ALIGNER_OPTIONS + ' -W ' + self.gets2tfilename('A3.final') + ':' + self.gett2sfilename('A3.final'), "Building alignment model",self.gets2tfilename('alignmodel.colibri') ): return False 
        else:  
